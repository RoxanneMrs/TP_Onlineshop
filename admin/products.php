...
$elemnForPaginationEnCours = pagination(
    $pdo,
    "page",
    "SELECT COUNT(id_product) as total FROM products",
    "total",
    5
);
...
if(isset($_GET["key_word"])) {
    ...
    $stmt = $pdo->query("SELECT * FROM products WHERE description LIKE '%$keyWord%' LIMIT $elemnForPaginationEnCours[premier], 5");

} else {

    $stmt = $pdo->query("SELECT * FROM products LIMIT $elemnForPaginationEnCours[premier], 5");
    ...
    <tbody>
        <?php while ($product = $stmt->fetch(PDO::FETCH_ASSOC)) { ?>
            <tr>
                ...
                <td> <a href="?action=modifier&id_product=<?= $product["id_product"]; ?>#form">Modifier</a> </td>
                <td> <a href="?action=supprimer&id_product=<?= $product["id_product"]; ?>">Supprimer</a> </td>
            </tr>
        <?php } ?>
        </tr>
